...
#
# Simplified version of "bin/setupvars.sh" in OpenVINO's directory
...
echo -e "[setupvars.sh] OpenVINO environment initialized\n"


# Activate real-time fashion detection


echo -e "Commencing real-time fashion detection\n"

python3 deployment_tools/open_model_zoo/demos/object_detection_demo/python/fashion_detection.py \
    -i 'http://pi:raspberry@192.168.12.150:8090/stream.mjpg' \
    --m main/models/yolov3-df2.xml \
    -at yolo \
    -ds df2 \
    --output_limit 0 \
    --save_detections main/logs/detections.json \
    -o main/logs/output_video.mp4 \
    -r -d CPU \
    --output_broker
